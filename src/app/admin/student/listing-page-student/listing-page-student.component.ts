  ...
  loadContent() {
    ...
    this.studentService.students(this.listingOption).subscribe({
      next: (result: ListingResult) => {
        ...
        // let departmentRequest = this.departments.length > 0
        // ? this.studentService.departments({ include: this.listingOption.departments.map(it=>it.id) }).pipe(map((it) => this.department.cityMunicipalityMapper(it?.result)))
        // : of([]);
